...
"""
function laser_factory(las::Laser,mp::MaterialParameters,dims=Homogenous()::Dimension)
    temporal = las(las::Laser)
    power = intensity(las)
    spatial = spatial_laser(las,dims,mp)
    args=(temporal,spatial,power)
    return :(*($(args...)))
end
"""
...
"""
function (::Rectangular)(las::Laser)
...
"""
function spatial_laser(las::Laser,slab::Dimension,mp::MaterialParameters)
    z_laser = spatial_z_laser(las,slab,mp)
    xy_laser = spatial_xy_laser(slab)
    args = (z_laser,xy_laser)
    return :(*($(args...)))
end
"""
    ...
    along the z-axis
"""
function spatial_z_laser(las::Laser,slab::Dimension,mp::MaterialParameters)
    if las.Transport == "Ballistic"
        if typeof(slab) == Homogenous
            return 1/mp.δb
        else
            zs = [x[end] for x in slab.spatial_grid]
            return 1/(mp.δb*(1-exp(-length(slab.spatial_grid)/mp.δb))).*exp.(-zs./mp.δb)
        end
    elseif las.Transport == "Optical"
        if typeof(slab) == Homogenous
            ...
            zs = [x[end] for x in slab.spatial_grid]
            return 1/(mp.ϵ*(1-exp(-length(slab.spatial_grid)/mp.ϵ))).*exp.(-zs./mp.ϵ)
        end
    elseif las.Transport == "Combined"
        if typeof(slab) == Homogenous
            ...
            zs = [x[end] for x in slab.spatial_grid]
            return 1/((mp.δb+mp.ϵ)*(1-exp(-length(slab.spatial_grid)/(mp.δb+mp.ϵ)))).*exp.(-zs./(mp.δb+mp.ϵ))
        end
    end
end
"""
