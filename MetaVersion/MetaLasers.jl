...
"""
function laser_factory(las::Laser,dims=Homogenous()::Dimension)
    temporal = las()
    power = intensity()
...
function spatial_z_laser(las::Laser,slab::Dimension)
    if las.Transport == "Ballistic"
        if typeof(slab) == Homogenous
            ...
            return :(1/(mp.δb*(1-exp(-length(dim.spatial_grid)/mp.δb))).*exp.(-$zs./mp.δb))
        end
    elseif las.Transport == "Optical"
        if typeof(slab) == Homogenous
            ...
            zs = [x[end] for x in slab.spatial_grid]
            return :(1/(mp.ϵ*(1-exp(-length(dim.spatial_grid)/mp.ϵ))).*exp.(-$zs./mp.ϵ))
        end
    elseif las.Transport == "Combined"
        if typeof(slab) == Homogenous
            ...
            zs = [x[end] for x in slab.spatial_grid]
            return :(1/((mp.δb+mp.ϵ)*(1-exp(-length(dim.spatial_grid)/(mp.δb+mp.ϵ)))).*exp.(-$zs./(mp.δb+mp.ϵ)))
        end
    end
end
"""
