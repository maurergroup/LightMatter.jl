...
function athem_particleconservation(DOS::Spline1D,Δfneqe,Δfneqh,egrid::Vector{Float64},μ::Float64)
    ...
    return solve(NonlinearProblem(f,1.0),SimpleKlement();abstol=1e-4,reltol=1e-4).u
end
...
function athem_electronelectronscattering(ftot)
    ftotspl = :(get_interpolate(mp.egrid,$ftot))
    τee = :(mp.τ*mp.FE^2 ./((mp.egrid.-μ).^2 .+(pi*cons.kB*Tel)^2))
    frel = :(find_relaxeddistribution(mp.egrid,get_internalenergyspl(μ,$ftotspl,mp.DOS),noe,mp.DOS,cons.kB))
...
function find_relaxeddistribution(egrid::Vector{<:Real},goal::Real,no_part::Real,DOS::Spline1D,kB::Real)
    f(u) = goal - find_temperatureandμ(u,no_part,DOS,kB)
    Temp = solve(ZeroProblem(f,1000.0),Order16();abstol=1e-6,reltol=1e-6)
