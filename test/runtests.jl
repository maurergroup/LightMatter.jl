...
using LightMatter, PreallocationTools, BenchmarkTools
...
@testset "AthermalElectrons" begin
    ...
    DOS = generate_DOS("../DOS/Au_DOS.dat", 1.0)
    tmp1 = DiffCache(similar(egrid))
    tmp2 = DiffCache(similar(egrid))
    ...
    fneq = zeros(length(egrid))
    n = LightMatter.get_thermalparticles(0.0, 1e-16, DOS, egrid)
    @test LightMatter.athem_electronelectronscattering(tmp1, tmp2, 300.0, 0.0, egrid, fneq, n, τee) == read(file["athemelectronelectronscattering"])
