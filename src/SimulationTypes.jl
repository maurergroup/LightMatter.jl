...
@kwdef struct Structure <: SimulationTypes
    ...
    dimension::Union{Dimension} # A struct holding all spatial grid structure (0D or 1D)
    fields::Fields # Any external fields in the simulation
...
function build_Structure(; las::Laser=build_Laser(), Spatial_DOS::Bool = false, Elemental_System::Int = 1, dimension::Dimension = build_Dimension(),
    ...
    ext_fields = Fields(tmp, tmp), bandstructure::Union{Symbol, Nothing} = nothing, FE=0.0, fields = false)
    ...
    return Structure(Spatial_DOS=Spatial_DOS, Elemental_System=Elemental_System, DOS=DOS, egrid=egrid, dimension=dimension, fields = ext_fields,
