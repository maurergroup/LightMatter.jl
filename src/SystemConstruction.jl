...
function generate_initialconditions(sim::Simulation,initialtemps::Dict{String, <:Real})
    ...
    if sim.electronictemperature.Enabled == true
        merge!(temp_u0, Dict("Tel" => fill(sim.structure.dimension.length, initialtemps["Tel"])))
    end
    if sim.phononictemperature.Enabled == true
        merge!(temp_u0, Dict("Tph" => fill(sim.structure.dimension.length, initialtemps["Tph"])))
    end
...
function generate_parameters(sim::Simulation,initialtemps::Dict{String, <:Real})
    ...
    if sim.ElectronicTemeprature == true && sim.electronictemperature.AthermalElectron_ElectronCoupling == false
        if typeof(sim.structure.DOS) == Vector{spl}
            no_part=zeros(sim.structure.dimension.length)
            for j in eachindex(sim.structure.dimension.grid)
                no_part[j] = get_thermalparticles(0.0,1e-32,sim.structure.DOS[j],sim.structure.egrid)
...
function conductivity_expressions(sim::Simulation)
    cond_exprs = []
    if sim.electronictemperature.Conductivity == true
        push!(cond_exprs,:(Lightmatter.electrontemperature_conductivity!(u.Tel,p.sim,u.Tph,p.Tel_cond)))
    end
    if sim.phononictemperature.Conductivity == true
        push!(cond_exprs,:(Lightmatter.phonontemperature_conductivity!(u.Tph,p.sim,p.Tph_cond)))
    end
    if sim.athermalelectrons.Conductivity== true
        push!(cond_exprs,:(Lightmatter.electron_distribution_transport!(p.mp.v_g,u.fneq,p.f_cond,p.dim)))
        if sim.athermalelectrons.AthermalElectron_ElectronCoupling == true
            push!(cond_exprs,:(n_cond = Lightmatter.thermal_particle_transport!(sim.athermal.v_g,p.mp.egrid,u.noe,p.dim)))
        end
    end 
    return cond_exprs
end
...
function build_loopbody(sys,sim::Simulation)
    ...
    push!(exprs, :(Lightmatter.find_chemicalpotential(n,Tel,DOS,sim.structure.egrid)))
