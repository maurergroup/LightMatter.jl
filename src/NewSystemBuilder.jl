function new_function_builder(sim,las,dim)
    ...
    if sim.ParameterApprox.EmbeddingMethod == true && sim.Systems==SystemComponents(true,true,true)
        ...
        comb_key_list =vcat(key_list_embed,key_list_bg)
        return comb_sys
    else
...
function new_initial_conditions(sim,mp,initialtemps,dim)
    if sim.Systems.NonEqElectrons == true
        ...
        elseif sim.Systems.PhononTemperature == false
            ...
            temp_u0 = Dict("noe"=>no_part, "fneq"=>zeros(dim.length,length(mp.egrid)), "Tel"=>fill(initialtemps[i],dim.length))
        else
            no_part=zeros(dim.length)
            for j in eachindex(dim.grid)
                no_part[j] = get_thermalparticles(0.0,1e-32,mp.DOS[j],8.617e-5,mp.egrid)
            end
            temp_u0 = Dict("noe"=>no_part, "fneq"=>zeros(dim.length,length(mp.egrid)), "Tel"=>fill(initialtemps[i],dim.length), "Tph"=>fill(initialtemps[i],dim.length))
        end
    else
        temp_u0 = Dict("Tel"=>fill(initialtemps[i],dim.length), "Tph"=>fill(initialtemps[i],dim.length))
    end
    namtup = NamedTuple((Symbol(key),value) for (key,value) in temp_u0)
    return NamedArrayPartition(namtup)
end

function new_parameters(sim,las,mp,initialtemps,dim)
    if sim.Systems.NonEqElectrons==true
        if sim.Systems.ElectronTemperature==false
...
function build_loopbody(sys,sim)
    ...
    push!(exprs,:(μ = find_chemicalpotential(u.noe,u.Tel,mp.DOS[i],cons.kB,mp.egrid)))
...
function variable_renaming(sim)
    old_name=[:(p.cond[i]),:(mp.DOS[i]),:(p.mp),:(p.las)]
    new_name=[:cond,:DOS,:mp,:las]
    if sim.Systems.NonEqElectrons == true
