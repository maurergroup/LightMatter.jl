...
function athemdistribution_factory(sim::Simulation, laser_expr::Expr)
    ...
    if sim.laser.hv isa Matrix
        athemexcite = :(vec(sum($laser_expr .* LightMatter.athemexcitation!(tmp1, tmp2, $ftot, sim.structure.egrid, DOS, sim.laser.hv, $M), dims=1)))
    else
        athemexcite = quote
                        LightMatter.athemexcitation!(tmp1, tmp2, $ftot, sim.structure.egrid, DOS, sim.laser.hv, $M)
                        $laser_expr .* tmp2
                      end
    end
...
function find_temperatureandμ!(du, u, (out, n, DOS, egrid, goal)) #Tel=u[1], μ=u[2]
    out = get_tmp(out, u[1])
