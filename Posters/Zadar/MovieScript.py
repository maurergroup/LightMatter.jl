...
cus_data=cus.postprocessing.total_distribution.distribution.nxvalue
...
def make_frame(t):
    ...
    cus_idx = min(range(len(cus_time)), key=lambda i: abs(cus_time[i]-sim_t))
    AthEM_idx = min(range(len(AthEM_time)), key=lambda i: abs(AthEM_time[i]-sim_t))

    cus_cp = cus.dynamics.material_0.position_0_0_0.electrons.band_0.total.abs_chemical_potential.abs_chemical_potential.nxvalue[cus_idx]
    cus_T = cus.postprocessing.temperature.temperature.nxvalue[cus_idx]
    cus_neqdis=cus_data[cus_idx,:] - FermiDirac(cus_energy,cus_cp,cus_T)
    ...
    cus_ehp = cus_DOS*cus_neqdis

    ax.clear()
    ax.text(-4,0.31,str(round(sim_t/1e-15,2))+' fs',fontsize=20)
    ax.plot(cus_energy[90:-100],cus_ehp[90:-100],label='Shifted DOS',linewidth=5,color='red')
    ax.plot(AthEM_energy[200:-100],AthEM_ehp[200:-100],label='Au DOS',linewidth=5,color='blue',dashes=[6,2])
    ax.set_xlabel(r'$\mathrm{E-E_F / eV}$',fontsize=24)
...
animation.write_videofile("ShiftedDOSehps.mp4",fps=10,codec='libx264',audio=False,threads=8,logger="bar")