...
function generate_DOS(File::String,n) #Simply builds a spline of a DOS froma  file and scales from eV^-1atom^-1 to eV^-1nm^-1
...
function discretization(dis::Vector{Float64}, n::Int, dos::Vector{Float64},grid) 
    ...
    tolerance = particle_number*0.001 #Defines an error tolerance of the particles of 1% - can tweak but may get stuck in infinite loop

    for j in 1:n
        current_particles = trapz(grid,binary_vectors[:, j] .* dos) #Calculates current number of particles of x binary distribution
        diff = particle_number - current_particles # Finds the difference

        while abs(diff) > tolerance #Checks if diff is greater than tolerance
            if diff > 0 #If diff > 0 then we want to edit the l.h.s of the distribution where most values should be 1 and if < 0 then edit the r.h.s 
                for i in 1:m
                    if abs(diff) <= tolerance
                        break
                    elseif binary_vectors[i, j] == 0 && dos[i] > 0 #Changes a value from 0 to 1 and recalculates difference
            ...
            elseif diff < 0
                for i in m:-1:1 
                    if abs(diff) <= tolerance
                        break
                    elseif binary_vectors[i, j] == 1 && dos[i] > 0 #Changes a value from 1 to 0 and recalculates difference
...
splits,parts = discretization(dis,vecs,DOS,NAH_energygrid) #Builds the vectors and returns the vectors (splits) and the relative particle distribution (parts)
